    ...
    const jiraAnswers = await inquirer.prompt([
        {
            ...
            validate: (input) => input.trim().length > 0 || 'El dominio es obligatorio',
        },
        {
            type: 'input',
            name: 'jiraEmail',
        ...
        },
        {
            ...
            mask: '*',
            validate: (input) => input.trim().length > 0 || 'El token es obligatorio',
        },
    ]);
    ...
    const bbAnswers = await inquirer.prompt([
        {
            ...
            validate: (input) => input.trim().length > 0 || 'El workspace es obligatorio',
        },
        {
            type: 'input',
